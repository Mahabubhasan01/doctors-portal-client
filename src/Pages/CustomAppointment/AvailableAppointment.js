...
const AvailableAppointment = ({ date }) => {
  ...
  const url = `https://hidden-sands-99680.herokuapp.com/available?date=${formattedDate}`;
  /* useEffect(() => {
        const url = `https://hidden-sands-99680.herokuapp.com/available?date=${formattedDate}`
  ...
  if (isLoading) {
    return (
      <button className="btn loading flex h-screen w-screen justify-center items-center">
        Loading
      </button>
    );
  }
